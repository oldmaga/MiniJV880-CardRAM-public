...
Default MIDI CC mapping in `src/minijv880.ini`:

| Function | CC | Notes |
| --- | ---: | --- |
| ENTER | 59 | Normal ENTER tap. Inside the PN-JV80/SYX browser, it enters the selected folder or loads the selected `.SYX` file. |
| DATA dial CW | 60 | One encoder-style step forward / clockwise. Works in normal screens and in SR, RD-500 and PN-JV80/SYX overlays. |
| DATA dial CCW | 61 | One encoder-style step backward / counter-clockwise. Works in normal screens and in SR, RD-500 and PN-JV80/SYX overlays. |
| SR overlay toggle | 62 | Opens/closes the MiniJV880 SR overlay without using the physical DATA button. |
| ENTER LONG | 63 | Opens the PN-JV80/SYX browser from normal UI contexts and works as back/exit while the browser is open. |
| ALL RELEASE / PANIC | 64 | Clears MIDI-held button states and timed ENTER requests. |

Typical PN-JV80/SYX browser test sequence:

1. Send **ENTER LONG / CC 63** to open the PN-JV80/SYX browser.
2. Use **DATA dial CW / CC 60** and **DATA dial CCW / CC 61** to move through folders and files.
3. Send **ENTER / CC 59** to enter a folder or load the selected `.SYX` file.
4. Send **ENTER LONG / CC 63** to go back from a subfolder or exit from the root browser level.

The MIDI Lab tab currently includes:

- **Generic CC sender**: sends one CC/value pair on the configured channel.
...
- DATA / CC49 is kept for completeness in the 2.4.0 branch, but the DATA CC event itself has no practical effect. Use DATA dial CW / DATA dial CCW for encoder-style DATA movement and SR overlay toggle for the MiniJV880 SR overlay.
